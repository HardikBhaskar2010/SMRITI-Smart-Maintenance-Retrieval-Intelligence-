...
Uses FastAPI TestClient with a temporary isolated ChromaDB directory so
...
# ── Point ChromaDB and uploads at temp dirs BEFORE importing the app ──
_tmp_root = tempfile.mkdtemp(prefix="smriti_test_")
os.environ.setdefault("OPENROUTER_API_KEY", "dummy_test_key_openrouter_xyz")
...
def seeded_asset_id(client):
    """
    Seeds a minimal asset collection directly into ChromaDB so that
    asset/query/guru tests have something to work with.
...
def minimal_pdf_bytes():
    ...
    # Minimal 1-page PDF with a text stream
    pdf = (
        b"%PDF-1.4\n"
